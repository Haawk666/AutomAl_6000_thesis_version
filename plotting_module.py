                                    ...
                                    if not (exclude_2 and vertex.flag_2):
                                        if not (exclude_3 and vertex.flag_3):
                                            if not (exclude_4 and vertex.flag_4):
                                                ...
                                                theta_mean = instance.graph.produce_theta_mean(vertex.i, exclude_angles_from_inconsistent_meshes=True)
                                                max_ = max(instance.graph.produce_alpha_angles(vertex.i))

                                                if vertex.species() == 'Cu':
                                                    self.cu_param.append(theta_mean)

                                                elif vertex.species() == 'Si':
                                                    if vertex.flag_2:
                                                        self.si_2_param.append(theta_mean)
                                                    else:
                                                        self.si_1_param.append(theta_mean)

                                                elif vertex.species() == 'Al':
                                                    if vertex.is_in_precipitate:
                                                        self.al_1_param.append(theta_mean)
                                                    else:
                                                        self.al_2_param.append(theta_mean)

                                                elif vertex.species() == 'Mg':
                                                    if max_ > 3.15:
                                                        self.mg_2_param.append(theta_mean)
                                                    else:
